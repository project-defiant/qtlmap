...
def make_var_info_dict(csv_file_path):
    df = pd.read_csv(
        ...
        header=None,
        dtype=str,
        names=["variant", "type", "ac", "an", "r2"],
        usecols=[2, 5, 6, 7, 9],
        verbose=True,
        keep_default_na=False,
    )
    df = df.assign(
        compact_info=df["type"].astype(str)
        + "#"
        + df["ac"].astype(str)
        + "#"
        + df["an"].astype(str)
...
def make_pheno_meta_dict(csv_file_path):
    df = pd.read_csv(
        ...
        sep="\t",
        dtype=str,
        usecols=["phenotype_id", "group_id", "gene_id"],
        verbose=True,
    )
    df = df.assign(
        compact_info=df["group_id"].astype(str) + "#" + df["gene_id"].astype(str)
    )
...
def make_median_tpm_dict(csv_file_path):
    df = pd.read_csv(
        ...
        sep="\t",
        dtype=str,
        usecols=["phenotype_id", "median_tpm"],
        verbose=True,
    )
...
def main():
    ...
    writer = csv.writer(
        io.TextIOWrapper(f_w, newline="", write_through=True), delimiter="\t"
    )
        ...
        with gzip.open(summ_stats, "rt") as f:
            ...
            for line in f:
                ...
                if k[5] in var_info_dict:
                    k.extend(
                        var_info_dict[k[5]].split("#")
                    )  # join ac,an,type, and r2 values
