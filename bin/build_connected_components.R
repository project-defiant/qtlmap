...
option_list <- list(
  ...
  make_option(
    ...
    type = "character",
    default = NULL,
    help = "Purity filtered susie output. Tab separated file",
    metavar = "type"
  ),
  make_option(
    c("-p", "--phenotype_meta"),
    type = "character",
    default = NULL,
    help = "Phenotype metadata file. Tab separated file",
    metavar = "type"
  ),
  make_option(
    c("-q", "--qtl_group"),
    type = "character",
    default = NULL,
    help = "qtl group of the given dataset",
    metavar = "type"
  ),
  make_option(
    c("-o", "--output_file"),
    type = "character",
    default = "./cc_lead_phenotypes.txt",
    help = "Output file path and name [default \"%default\"]",
    metavar = "type"
  ),
  make_option(
    c("-c", "--cs_size_threshold"),
    type = "integer",
    default = 200,
    help = "Threshold to filter out credible sets bigger than given size",
    metavar = "type"
  )
)
...
message(
  " ## Loading libraries: devtools, dplyr, SummarizedExperiment, cqn, data.table"
)
...
make_connected_components_from_cs <- function(
  ...
  cs_ranges = GenomicRanges::GRanges(
    ...
    strand = "*",
    mcols = data.frame(
      cs_uid = susie_filt$cs_uid,
      variant_id = susie_filt$variant,
      gene_id = susie_filt$molecular_trait_id
    )
  )
  ...
  olaps <- olaps %>%
    dplyr::mutate(
      cs_uid_1 = cs_ranges$mcols.cs_uid[queryHits],
      cs_uid_2 = cs_ranges$mcols.cs_uid[subjectHits]
    )
  edge_list <- olaps %>%
    dplyr::select(cs_uid_1, cs_uid_2) %>%
    BiocGenerics::unique() %>%
    base::as.matrix()
...
susie_naive <- readr::read_tsv(
  file = susie_file_path,
  col_types = "cccicccccdddddddd"
)
...
susie_naive <- susie_naive %>%
  dplyr::mutate(cs_uid = paste0(qtl_group_in, "%", cs_id))
susie_naive_cc <- make_connected_components_from_cs(
  susie_all_df = susie_naive,
  cs_size_threshold = cs_size_threshold_in
)

susie_naive_filt_cc <- susie_naive %>%
  dplyr::filter(molecular_trait_id %in% susie_naive_cc$molecular_trait_id) %>%
  dplyr::left_join(
    susie_naive_cc %>% dplyr::select(cc_membership_no, molecular_trait_id),
...
susie_high_pip_with_gene <- susie_naive_high_pip_var %>%
  dplyr::left_join(
    phenotype_metadata %>% dplyr::select(-chromosome),
...
readr::write_tsv(
  x = susie_high_pip_with_gene %>%
    dplyr::select(molecular_trait_id) %>%
    dplyr::distinct(),
  file = output_dir,
  col_names = F
)
readr::write_tsv(
  x = susie_high_pip_with_gene,
  file = paste0(qtl_group_in, "_with_stats.tsv")
)
