...
convertDFtoQTLtools <- function(
  sample_meta_qtlgroup,
  count_matrix,
  phenotype_data
) {
  ...
  count_matrix_group <- count_matrix[,
    colnames(count_matrix) %in%
      c(sample_meta_qtlgroup$sample_id %>% as.character(), "phenotype_id")
  ]
  match_index <- match(
    names(count_matrix_group),
    sample_meta_qtlgroup$sample_id
  )
  names(count_matrix_group)[
    !is.na(match_index)
  ] <- as.character(sample_meta_qtlgroup$genotype_id[na.omit(match_index)])
  ...
  pheno_data_filtered <- pheno_data %>%
    dplyr::filter(phenotype_id %in% count_matrix_group$phenotype_id)
...
importVariantInformation <- function(path) {
  ...
  snp_info = utils::read.delim(
    path,
    colClasses = into_col_types,
    col.names = info_col_names,
    header = FALSE
  ) %>%
    as.data.frame()
  assertthat::assert_that(nrow(snp_info) > 1, msg = "variant info is empty")

  snp_info = dplyr::mutate(
    snp_info,
    indel_length = pmax(nchar(alt), nchar(ref))
  ) %>%
...
performQTLGroupPCA <- function(ph_count_matrix_group, output_dir) {
  ...
  pheno_pca <- stats::prcomp(
    t(ph_count_matrix_group[-(1:6)]),
    center = TRUE,
    scale. = TRUE
  )
  ...
  pheno_pca_x <- cbind(
    SampleID = paste0("pheno_", rownames(pheno_pca_x)),
    pheno_pca_x
  )

  # write phenotype PCA matrix into file
  message(
    " ## write phenotype PCA matrix to ",
    file.path(output_dir, "pheno_PCA.tsv")
  )
  write.table(
    pheno_pca_x,
    file = file.path(output_dir, "pheno_PCA.tsv"),
    quote = FALSE,
    sep = "\t",
    row.names = FALSE,
    col.names = TRUE
...
option_list <- list(
  ...
  optparse::make_option(
    ...
    type = "character",
    default = NULL,
    help = "Variant information file path.",
    metavar = "type"
  ),
  optparse::make_option(
    c("-o", "--outdir"),
    type = "character",
    default = "./QTLTools_input_files",
  ...
  optparse::make_option(
    ...
    type = "integer",
    default = 5,
    help = "Minimum count of cis variants in cis-distance of gene to be taken into account. [default \"%default\"]",
    metavar = "number"
  ),
  optparse::make_option(
    c("-a", "--add-covariates"),
    type = "character",
    default = "null",
    help = "Comma-separate list of additonal covariate columns names to be included in the analysis. [default \"%default\"]",
    metavar = "type"
  )
)
...
phenotype_data <- utils::read.delim(
  phenotype_meta_path,
  quote = "",
  header = TRUE,
...
message(
  "Keep samples that are present both in the expression data and sample metdata"
)
...
olap_count = GenomicRanges::countOverlaps(
  gene_ranges,
  var_ranges,
  ignore.strand = TRUE
)
count_df = dplyr::select(
  phenotype_data,
  phenotype_id,
  chromosome,
  phenotype_pos
...
phenotype_data <- phenotype_data[
  phenotype_data$phenotype_id %in% count_df$phenotype_id,
]
count_matrix_cis_filter <- count_matrix[
  count_matrix$phenotype_id %in% count_df$phenotype_id,
]
...
assertthat::assert_that(
  length(qtl_groups) == 1,
  msg = "Number of distinct qtl_groups exceeds 1. Please check your sample metadata."
)
qtl_group = qtl_groups[1]

message(" ## Prepare data for QTLtools/FastQTL")
qtltools_df = convertDFtoQTLtools(
  sample_metadata,
...
file_path = file.path(
  output_dir,
  paste(qtl_group, "sample_names.txt", sep = ".")
)
write.table(
  sample_names,
  file_path,
...
utils::write.table(
  pheno_pca_x,
  file = file.path(output_dir, "phenoPCA.tsv"),
  quote = FALSE,
  sep = "\t",
  row.names = FALSE,
  col.names = TRUE
...
if (!(covariate_names == "null")) {
  ...
  utils::write.table(
    cov_df,
    file = file.path(output_dir, "additional_covariates.tsv"),
    quote = FALSE,
    sep = "\t",
    row.names = FALSE,
    col.names = TRUE
