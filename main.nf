...
workflow {
  ...
  if (workflow.profile == 'awsbatch') {
    ...
    if (!params.awsqueue || !params.awsregion) {
      exit(1, "Specify correct --awsqueue and --awsregion parameters on AWSBatch!")
    }
    if (!workflow.workDir.startsWith('s3') || !params.outdir.startsWith('s3')) {
      exit(1, "Specify S3 URLs for workDir and outdir parameters on AWSBatch!")
    }
    // Check workDir/outdir paths to be S3 buckets if running on AWSBatch
    // related: https://github.com/nextflow-io/nextflow/issues/813
    if (!workflow.workDir.startsWith('s3:') || !params.outdir.startsWith('s3:')) {
      exit(1, "Workdir or Outdir not on S3 - specify S3 Buckets for each to run on AWSBatch!")
    }
  }
  ...
  */
  ...
  channel.fromPath(params.studyFile)
    ...
    .splitCsv(header: true, sep: '\t', strip: true)
    .map { row ->
      def tpm = row.containsKey('tpm_file') && row.tpm_file ? file("${baseDir}/${row.tpm_file}") : null
  ...
  if (params.run_nominal) {
    ...
    generate_sumstat_batches_input_ch = chr_rsid_map_ch
      .cross(nominal_qtl_subset_info_correct_format_ch)
      .map { rsid_data, nominal_data ->
